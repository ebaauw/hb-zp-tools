...
class ZpClient extends HttpClient {
  ...
  _handleMessage (message) {
    if (message.service === 'ZoneGroupTopology') {
      this._zoneGroupState = message.body
      this.emit('gotcha')
    }
  }
      ...
      for (const group of this._zoneGroupState.zoneGroups) {
        ...
        for (const member of group.zoneGroupMembers) {
          ...
          if (props.id === this._params.id) {
            this.#checkAddress(props.address)
    ...
    this._zpListener.on(this.id, async (message) => {
      try {
        ...
        if (message.body != null) {
          ...
          message.body = await this._parser.parse(message.body)
        }
        if (
          message.service === 'ZoneGroupTopology' &&
          message.body.zoneGroups != null
        ) {
          this._zoneGroupState = message.body
        ...
        this.vvdebug(
          '%s: notify %s/Event: %j', this.name, notify, message.rawBody
        )
        this.vdebug(
          '%s: notify %s/Event: %j', this.name, notify, message.body
        )
        this.debug('%s: notify %s/Event', this.name, notify)
  ...
  async subscribe (url) {
    ...
    try {
      const response = await this.#request('SUBSCRIBE', url, undefined, headers)
  ...
  async unsubscribe (url) {
    ...
    if (sid != null) {
      try {
        await this.#request('UNSUBSCRIBE', url, undefined, { SID: sid })
      } catch (error) { this.error(error) }
    }
  ...
  async _checkBootSeq (bootSeq) {
    ...
    this.debug(
      '%s: rebooted (%d -> %d)', this.name,
      oldBootSeq, this.bootSeq
    )
  ...
  #checkAddress (address) {
    ...
    this.debug(
      '%s: IP address changed (%s -> %s)', this.name,
  ...
  async handleAliveMessage (message) {
    ...
    this.#checkAddress(message.address)
  ...
  async updateAlarm (alarm) {
    return this.post('ZonePlayer', 'AlarmClock', 'UpdateAlarm', {
      ...
      // Note that when set from the Sonos iOS app, the alarm includes a
      // <Content type="..." .../> tag.  This tag is lost when updating the
      // alarm here, but also when updating from the Sonos macOS app.
    })
  }
  ...
  static meta (metaData, albumArtUri, description) {
    ...
    meta += `<upnp:class>${metaData.class}</upnp:class>`
    if (albumArtUri != null) {
  ...
  async post (device, service, action, options) {
    ...
    const response = await this.#request('POST', url, body, headers, info)
    await this.#updateLastSeen()
  ...
  async #request (method, resource, body, headers, info) {
