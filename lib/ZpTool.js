// hb-zp-tools/lib/ZpTool.js
// Copyright © 2019-2026 Erik Baauw. All rights reserved.
//
// Command line interface to Sonos ZonePlayer.
...
import { ZpClient } from './ZpClient.js'
...
const usage = {
  zp: `${b('zp')} [${b('-hVD')}] [${b('-H')} ${u('hostname')}[${b(':')}${u('port')}]] [${b('-p')} ${u('port')}] [${b('-t')} ${u('timeout')}] ${u('command')} [${u('argument')} ...]`,
  info: `${b('info')} [${b('-hnv')}]`,
  ...
  listAlarms: `${b('listAlarms')} [${b('-hns')}]`,
  alarm: `${b('alarm')} [${b('-h')}] ${u('id')} [${b('on')}|${b('off')}]`
}
...
const description = {
  ...
  listAlarms: 'Get alarms.',
  alarm: 'Get/enable/disable alarm.'
}
...
Parameters:
  ...
  ${b('-p')} ${u('port')}, ${b('--port=')}${u('port')}
  Set listener port to ${u('port')}.  Default: random.
...
Parameters:
  ...
  listAlarms: `${description.listAlarms}

Usage: ${b('zp')} ${usage.listAlarms}
...
class ZpTool extends CommandLineTool {
  constructor (pkgJson) {
    super()
  ...
  async main () {
    try {
      ...
      this.zpListener = new ZpListener(this.listenerPort)
      this.zpListener
      ...
      this._clargs.options.logger = this
  ...
  parseArguments () {
    ...
    parser
      ...
      .option('p', 'port', (value) => {
        this.listenerPort = OptionParser.toInt('port', value, 0, 65535)
      })
      ...
      .on('message', (message) => {
        if (clargs.topology) {
          ...
          this.log(
            ...
            jsonFormatter.stringify(message.body)
          )
        }
      })
    if (clargs.topology) {
  ...
  async listAlarms (...args) {
    ...
    parser
      ...
      .flag('n', 'noWhiteSpace', () => { clargs.options.noWhiteSpace = true })
      .flag('s', 'sortKeys', () => { clargs.options.sortKeys = true })
      .parse(...args)
    const jsonFormatter = new JsonFormatter(clargs.options)
    const result = await this.zpClient.listAlarms()
    const json = jsonFormatter.stringify(result)
    this.print(json)
  }
  ...
  async alarm (...args) {
    ...
    parser
      .help('h', 'help', this.help)
      .parameter('id', (value) => {
        clargs.id = OptionParser.toInt('id', value, 0, Infinity, true)
      })
      .remaining((list) => {
        if (list.length > 1) {
          throw new UsageError('too many arguments')
        ...
        if (list.length === 1) {
          clargs.enabled = OptionParser.toBool('enabled', list[0], true)
        }
      })
      .parse(...args)
    const response = await this.zpClient.listAlarms()
    const alarm = response?.currentAlarmList?.filter(
      (alarm) => { return alarm.id === clargs.id }
    )?.[0]
    if (alarm == null) {
      throw new UsageError(`${clargs.id}: alarm not found`)
    }
    if (clargs.enabled != null) {
      alarm.enabled = clargs.enabled ? 1 : 0
      await this.zpClient.updateAlarm(alarm)
    }
    this.print(alarm.enabled ? 'on' : 'off')
  }
